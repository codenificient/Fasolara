...
const resolvers = {
	Query: {
		accounts: async ( _, __, ) => await Account.find(),
		account: async ( _, { id }, ) => await Account.findById( id ),
		currentAccount: combineResolvers( isAuthenticated, async ( _, __, { userId } ) =>
		{
			try
			{
	...
	Mutation: {
		...
		deleteAccount: async ( _, { id } ) => await Account.findByIdAndRemove( id )
	},
	Subscription: {
		accountCreated: {
			subscribe: ( _, __, ___ ) => pubsub.asyncIterator( 'accountCreated' ),
		},
		accountUpdated: {
			subscribe: ( _, __, ___ ) => pubsub.asyncIterator( 'accountUpdated' ),
		},
		accountDeleted: {
			subscribe: ( _, __, ___ ) => pubsub.asyncIterator( 'accountDeleted' ),
		},
	}
}
